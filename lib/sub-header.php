<?php
    ...
    $subtext = '';
    global $post;
    if(!function_exists('backnow_call_sub_header')){
        function backnow_call_sub_header(){
            $banner_img = get_theme_mod( 'sub_header_banner_img', false );

            $banner_color = get_theme_mod( 'sub_header_banner_color', '#2ac1af' );
                ...
                if(!empty($image_attached)){
                    ...
                    if(empty($sub_img[0])){
                        ...
                        if( get_post_meta(get_the_ID(),"themeum_subtitle_color",true ) == ''){
                            $output = backnow_call_sub_header();
                        }
                    }
                }else{
            ...
            <div class="col-12 text-center">
                    ...
                    if(isset($wp_query->queried_object->name)){
                        if (get_theme_mod( 'header_title_enable', true )) {
                            if($wp_query->queried_object->name != ''){
                                if($wp_query->queried_object->name == 'product' ){
                                    echo '<h2>'.esc_html__('Shop','backnow').'</h2>';
                                }else{
                                    ...
                                    if($category) {
                                        ...
                                        if ($option_name) {
                                            echo '<span class="backnow-icons"><i class="back-'.get_option($option_name).'"></i></span>';
                                        }
                        ...
                        if( is_search() ){
                            ...
                            if (get_theme_mod( 'header_title_enable', true )) {
                                $first_char = esc_html__('Search','backnow');
                            ...
                            if (get_theme_mod( 'header_title_enable', true )) {
                                if (get_post_type() == 'event') {
                                    echo '<h2 class="page-leading">'. esc_html__( 'Event Details','backnow' ).'</h2>';
                                } elseif (get_post_type() == 'album') {
                                    echo '<h2 class="page-leading">'. esc_html__( 'Albums','backnow' ).'</h2>';
                                } elseif (get_post_type() == 'gallery') {
                                    echo '<h2 class="page-leading">'. esc_html__( 'Gallery','backnow' ).'</h2>';
                                } elseif (get_post_type() == 'performer') {
                                    echo '<h2 class="page-leading">'. esc_html__( 'Performer','backnow' ).'</h2>';
